...
class EC2ClosePort5439(object):
    ...
    def remove_port(
        ...
        for rule in security_group_rules:
            ...
            if (
                ...
                # If FromPort is equal and ToPort is greater that the port no. which is to be
                # removed then modify the security group rule by removing the port from the range
                elif rule["FromPort"] == port and rule["ToPort"] > port:
                    if not self.check_if_port_range_exists(
                ...
                # If FromPort is less than and ToPort is greater that the port no. which is to be removed then modify the security group rule
    ...
    def remediate(self, client, instance_id):
        """Block public access to port 5439 of all security groups attached to an EC2 instance.
