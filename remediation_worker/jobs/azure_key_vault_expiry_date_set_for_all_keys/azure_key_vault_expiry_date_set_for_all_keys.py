...
class SetExpirationForKey(object):
    ...
    def ensure_key_permissions(
        ...
        """Ensures that appropriate key permissions are given to the app to update it.
        ...
        if self.check_rbac_enabled(
            ...
            # Update Key Vault Access policy to provide access for the application to update Key
            logging.info(
                "Updating Key Vault Access policy to provide access for the application to set Key Expiry date"
            )
    ...
    def create_role_assignment(
        ...
        """
        logging.info(
            f"Creating a Role Assignment for Key Vault {key_vault_name} and assigning Key Vault Crypto User Role to the app"
        )
        ...
        client_authorization.role_assignments.create(
            ...
            parameters=RoleAssignmentCreateParameters(
                properties=RoleAssignmentProperties(
                    role_definition_id=f"/subscriptions/{subscription_id}/providers/Microsoft.Authorization/roleDefinitions/12338af0-0e69-4776-bea7-57ae8d297424",
    ...
    def update_key_vault_access_policy(
        ...
        access_policy_app = AccessPolicyEntry(
            ...
            permissions=Permissions(
                keys=[KeyPermissions.GET, KeyPermissions.LIST, KeyPermissions.UPDATE],
            ),
        )
    ...
    def remediate(
        ...
        :param tenant_id: Azure Tenant ID.
        :param key_client: Instance of the Azure KeyClient.
        ...
        try:
            ...
            # Check if the required permissions to update the key is given to the application, if not then give the required permissions.
            self.ensure_key_permissions(
                keyvault_client,
                client_authorization,
                key_vault_name,
                resource_group_name,
            ...
            logging.info("Setting Expiry date for Key")
            d = (
            ...
            logging.info("executing key_client.update_key_properties")
            key_client.update_key_properties(
                name=key_name, expires_on=expires_on,
            )
...
if __name__ == "__main__":
    sys.exit(SetExpirationForKey().run(sys.argv))
