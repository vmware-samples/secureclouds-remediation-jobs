...
This job encrypts the attached ebs volume.

## Getting Started

### Prerequisites

The provided AWS credential must have access to `ec2:*`.
...
```shell script
  ...
  python3 ebs_private_snapshot.py "`cat ../finding/finding.json`"
  where finding.json has volume id and region info
  {
    "notificationInfo": {
        "FindingInfo": {
            "ObjectId": "vol-06be1a14b20aecdfc",
            "Region": "us-west-2"
        }
    }
}
