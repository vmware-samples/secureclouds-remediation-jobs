...
from remediation_worker.jobs.azure_sql_threat_detection_types_all_server.azure_sql_threat_detection_types_all_server import (
    SetAdvanceThreatProtectionToAll,
)
...
class TestSetSqlServerThreatDetectionToAll(object):
    def test_parse_payload(self, valid_payload):
        params = SetAdvanceThreatProtectionToAll().parse(valid_payload)
    ...
    def test_remediate_success(self):
        client = Mock()
        action = SetAdvanceThreatProtectionToAll()
        ...
        assert updated_sql_server_parameters.disabled_alerts == []
