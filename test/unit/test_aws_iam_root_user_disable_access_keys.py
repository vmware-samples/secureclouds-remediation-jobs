...
from remediation_worker.jobs.aws_iam_root_user_disable_access_keys.aws_iam_root_user_disable_access_keys import (
    InactiveRootAccessKeys,
)
...
class TestInactiveRootAccessKeys(object):
    def test_parse_payload(self, valid_payload):
        params = InactiveRootAccessKeys().parse(valid_payload)
        assert params["root_user_name"] == "root_user_name"

    def test_remediate_success(self):
        client = Mock()
        action = InactiveRootAccessKeys()
        assert action.remediate(client, "root_user_name") == 0

    def test_remediate_with_exception(self):
        client = Mock()
        action = InactiveRootAccessKeys()
        with pytest.raises(Exception):
            assert action.remediate(client, "root_user_name")
